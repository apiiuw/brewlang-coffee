...
<div class="max-w-5xl mx-auto animate-fade-in-up">
    <div class="mb-6">
        <a href="{{ route('staff.orders.index') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-stone-500 hover:text-amber-400 transition">
    ...
    @if(session('success'))
        <div class="alert-success-dark mb-6 flex items-center gap-3">
            <i class="fa-solid fa-circle-check text-emerald-400 flex-shrink-0"></i>
            {{ session('success') }}
        </div>
    @endif
    @if(session('error'))
        <div class="alert-error-dark mb-6 flex items-center gap-3">
    ...
    <div class="grid gap-6 lg:grid-cols-[1fr_300px]">
        ...
        <aside class="rounded-2xl border border-stone-800 bg-stone-900 p-6 h-fit">
            <h2 class="font-semibold text-stone-200 mb-2">
                <i class="fa-solid fa-arrow-right-arrow-left text-amber-400/60 mr-2 text-sm"></i>
                Update Status
            </h2>
            <p class="text-xs text-stone-600 mb-5">Only forward transitions are allowed.</p>
            ...
            @else
                <x-empty-state
                    ...
                    description="This order has reached its final status."
                />
            @endif
